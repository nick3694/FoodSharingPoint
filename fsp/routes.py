...
from fsp import app, db, bcrypt, mail
from fsp.forms import (RegistrationForm, LoginForm, ContactForm,
                       RequestResetForm, ResetPasswordForm,CompleteOrderForm,ConfirmOrderForm,VolunteerForm,AddFood,RemoveFood,GiveBonusForm)
from fsp.models import (User,Point,FoodAvailable,Order,Bonus)

"This is The Information System class in which the association between a URL and the functions are handled, the routing between html pages and the server functions represents the main working logic" \
"There are a total of 26 function routes"
...
def send_reset_email(user, subject):
...
def volunteerequest():
    ...
    if current_user.is_authenticated:
         form = VolunteerForm()
         points = [point.name for point in Point.query.all()]
         adresses = [str(point.address) for point in Point.query.all()]
         supermarkets=zip(points,adresses)
         succes = False
         if request.method == 'POST':
             if form.validate_on_submit():
                ...
                %s %s is asking to becoming a volunteer in %s point          """ % (
                 ...
                 mail.send(msg)
                 flash("Thank you! You have requested to become our volunteer. We'll get back to you shortly.")
                 return render_template('volunteerrequest.html',title='Become a volunteer | FSP',supermarkets=supermarkets, form=form, points=points, succes=True,flag_access_as_volunteer=flag_access_as_volunteer)

         elif request.method == 'GET':
             return render_template('volunteerrequest.html',title='Become a volunteer | FSP',supermarkets=supermarkets, form=form, succes=succes, points=points,flag_access_as_volunteer=flag_access_as_volunteer,bonus=bonus)
...
def addfood():
    ...
    if form.validate_on_submit():
        ...
        datetime_object = datetime.strptime(str(datas), '%Y-%m-%d').date()
        today = date.today()
    ...
    if form.validate_on_submit():
        ...
        if bonuss:
            if form.bonus.data>0:
                ...
                form.first_name.data, form.last_name.data, form.bonus.data)
                mail.send(msg)
                today = date.today()
                bonus_added = Bonus(date_bonus=today, user_id=bonuss.id, bonus=form.bonus.data)
                ...
                if abs(bonuss.bonus) >= abs(form.bonus.data):
                    ...
                    form.first_name.data, form.last_name.data, voucher_quantity, tuple(voucher))
                    mail.send(msg)
                    today = date.today()
                    bonus_added = Bonus(date_bonus=today, user_id=bonuss.id, bonus=form.bonus.data, voucher=voucher_quantity)
                    db.session.add(bonus_added)
                    db.session.commit()
                else:
